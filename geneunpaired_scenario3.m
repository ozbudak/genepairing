...
% Reaction rates.
msh1=5; msh7=5; mdh1=0.4; mdh7=0.6; % mRNA synthesis and degradation rates.
...
for i=1:maxi-1 % Run the stochastic simulationm for max number of iterations.
  ...
  if RN==17
     mh1=mh1+2;%randi([1,2],1);
     s1=s1(1,2:Td1);
     Td1=Td1-1;
  end

  if RN==18
     mh7=mh7+3;%randi([1,2],1);
     s2=s2(1,2:Td2);
     Td2=Td2-1;
  end
  ...
  if RN==1 % Genes her1 and her7 are unpaired here
     s1 = [s1(1:Td1 - 1), nmh1, inf]; 
     Td1 = Td1 + 1;
  end
  
  if RN==2 % Genes her1 and her7 are unpaired here
...
writetable(DataTable, strcat('GeneUnpairedDFRv2/GeneUnpairedDFRv2_Run', num2str(run),'.xlsx'),'WriteVariableNames', true);
...
xlabel('Time')
ylabel('#mRNA')
saveas(gcf,strcat(['GeneUnpairedDFRv2/GeneUnpairedDFRv2_Run', num2str(run)]),'jpg');
close(gcf);

figure
histogram(mh1v)
saveas(gcf,strcat('GeneUnpairedDFRv2/GeneUnpairedDFRv2_Histogram', num2str(run)),'jpg');
close(gcf)
...
writetable(Stats, strcat('GeneUnpairedDFRv2/GeneUnpairedDFRv2_PearsonSpearman.xlsx'),'WriteVariableNames', true);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
